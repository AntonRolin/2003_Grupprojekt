...
function populateProductColumns(item) {
    let divElement = document.createElement("div");
    divElement.className = "col-md-4 pb-5";
    divElement.innerHTML = `<div class="my-3 ms-2 text-center"><p class="lead text-danger fs-2 fw-bold"><button type="button" id="${item.id}" class="buyButton btn btn-outline-success" onclick="sendOrderDetails(${item.id})">${item.id} ${item.name}</button></p><hr></div>`;
